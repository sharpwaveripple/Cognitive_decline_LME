...
df$agesq <- df$age06c**2
df$agemc <- df$age06 - mean(df$age06)

###############################
# Spaghetti plots
###############################

wmh.hv.plot <- ggplot(df, aes(x=log(wmh), y=hv, group=rundmcs)) +
  geom_line() +
  geom_point() +
  stat_smooth(aes(group=1), method=lm, colour="#cc0000", fill="#c3c3c3", alpha=0.6)+ 
  theme_bw(base_size=22)
plot(wmh.hv.plot)
lm.wmh.hv = lm(hv ~ log(wmh), data=df)
lm.wmh.hv.corr = lm(hv ~ log(wmh) + age06c + sex, data=df)
summary(lm.wmh.hv)
summary(lm.wmh.hv.corr)


time.cognind.plot <- ggplot(df, aes(x=time, y=cognitiveindex, group=rundmcs)) +
  geom_line() +
  geom_point() +
  stat_smooth(aes(group=1), method=lm, colour="#cc0000", fill="#c3c3c3", alpha=0.6) +
  theme_bw(base_size=22)
...
age.cognind.plot <- ggplot(df, aes(x=age, y=cognitiveindex, group=rundmcs)) +
  geom_line() +
  geom_point() +
  stat_smooth(aes(group=1), method=lm, colour="#cc0000", fill="#c3c3c3", alpha=0.6) +
  theme_bw(base_size=22)
...
m1.wmh = lmer(log(wmh) ~ time + agemc + sex + (1 + time|rundmcs),
              data=df, REML=FALSE, na.action=na.exclude)

m1.5.wmh = lmer(log(wmh) ~ time + age06c + sex + (1 + time|rundmcs),
              data=df, REML=FALSE, na.action=na.exclude)

m2.wmh = lmer(log(wmh) ~ time + age06c + timesqrt + sex + (1 + time|rundmcs),
              data=df, REML=FALSE, na.action=na.exclude)
summary(m0.wmh)
summary(m1.wmh)
anova(m1.wmh, m1.5.wmh)
